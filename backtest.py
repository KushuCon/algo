...
USAGE:
  python backtest.py --strategy momentum    --days 365
  python backtest.py --strategy dip_rider   --days 365
  python backtest.py --strategy rs_breakout --days 180
  python backtest.py --strategy momentum --symbols SNDK MU AMD NVDA --days 365
"""
...
# ── Strategy imports (only what exists) ───────────────────────────────────────
from strategies.momentum    import MomentumStrategy
from strategies.rs_breakout import RSBreakoutStrategy
from strategies.dip_rider   import DipRiderStrategy

# Archived strategies — import only if the files exist, skip otherwise
try:
    from strategies.archieve.rsi_mean_revert    import RSIMeanRevert
    from strategies.archieve.ma_crossover_sl    import MACrossoverWithSL
    from strategies.archieve.random_forest_strat import RandomForestStrategy
...
def _alpaca_keys_configured() -> bool:
    ...
    return bool(key and secret and key not in bad and secret not in bad)


def _normalize_ohlcv(df: pd.DataFrame, symbol: str | None = None) -> pd.DataFrame:
    """Flatten yfinance MultiIndex → lowercase open/high/low/close/volume."""
    if df is None or df.empty:
        return pd.DataFrame()
    ...
    return out[list(keep)].dropna(how="all") if keep else pd.DataFrame()


def _fetch_yfinance(symbols: list, days: int) -> dict:
    """Batch Yahoo download, then per-symbol normalize."""
    period   = f"{days + 90}d"
    all_bars = {s: None for s in symbols}

    log.info(f"Downloading {len(symbols)} symbol(s) from Yahoo ({period})...")
    time.sleep(0.5)

    if len(symbols) == 1:
        raw  = yf.download(symbols[0], period=period, progress=False,
    ...
    if isinstance(raw.columns, pd.MultiIndex):
        tickers_l0 = set(raw.columns.get_level_values(0).astype(str))
        for sym in symbols:
            if sym not in tickers_l0:
                continue
            norm = _normalize_ohlcv(raw[sym].copy(), sym)
            if not norm.empty:
                all_bars[sym] = norm.tail(days)
...
def _fetch_alpaca(symbols: list, days: int,
                  timeframe: str = "1Day", bar_limit: int = 500) -> dict:
    """Historical bars from Alpaca."""
    from broker import AlpacaBroker

    broker   = AlpacaBroker()
    limit    = days + 60 if timeframe == "1Day" else max(
        bar_limit, days * _BARS_PER_TRADING_DAY.get(timeframe, 78) + 50
    )
    all_bars = {}
    ...
    for sym in symbols:
        ...
        keep         = [c for c in _OHLCV_COLS if c in bars.columns]
        bars         = bars[keep].dropna(how="all")
        tail_n = days if timeframe == "1Day" else min(
            days * _BARS_PER_TRADING_DAY.get(timeframe, 78), len(bars), bar_limit * 2
        )
        all_bars[sym] = bars.tail(tail_n)
        log.info(f"  {sym}: {len(all_bars[sym])} bars (Alpaca)")
...
def fetch_history(symbols: list, days: int, source: str = "auto",
                  timeframe: str = "1Day", bar_limit: int = 500) -> dict:
    """
    Smart fetch: Yahoo for SYMBOLS_YAHOO, Alpaca for SYMBOLS_ALPACA.
    Falls back to Alpaca for any Yahoo failures.
    """
    symbols  = list(symbols)
    all_bars = {s: None for s in symbols}
    intraday = timeframe != "1Day"
    ...
    if intraday and source in ("auto", "yfinance"):
        log.info(f"Intraday {timeframe}: forcing Alpaca (yfinance skipped).")
        source = "alpaca"

    # ── Smart split: Yahoo handles the "liquid 8", Alpaca handles the "new 7" ──
    yf_candidates  = getattr(config, "SYMBOLS_YAHOO",  [])
    alp_candidates = getattr(config, "SYMBOLS_ALPACA", [])

    yf_syms  = [s for s in symbols if s in yf_candidates]
    alp_syms = [s for s in symbols if s in alp_candidates]
    other    = [s for s in symbols if s not in yf_candidates + alp_candidates]

    if source in ("auto", "yfinance") and not intraday and HAS_YF:
        try:
            fetched = _fetch_yfinance(yf_syms + other, days)
            all_bars.update(fetched)
        except Exception as e:
            log.warning(f"Yahoo batch failed: {e}")

    # Alpaca: dedicated symbols + any Yahoo failures
    still_missing = [s for s in symbols
                     if not (all_bars.get(s) is not None
                             and not all_bars[s].empty)]
    alp_needed = list(set(alp_syms) | set(still_missing))

    if alp_needed and source in ("auto", "alpaca") and _alpaca_keys_configured():
        log.info(f"Alpaca fetch for: {alp_needed}")
        all_bars.update(_fetch_alpaca(alp_needed, days, timeframe, bar_limit))

    # Warn on any remaining gaps
    for sym in symbols:
        df = all_bars.get(sym)
        if df is None or (isinstance(df, pd.DataFrame) and df.empty):
            log.warning(f"No data for {sym}")
...
def run_backtest(strategy, all_bars: dict,
                 ...
                 max_position_pct: float = config.MAX_POSITION_PCT,
                 stop_loss_pct: float    = config.STOP_LOSS_PCT,
                 take_profit_pct: float  = config.TAKE_PROFIT_PCT) -> dict:
    """
    Walk-forward simulation: day-by-day, no look-ahead.
    Positions stored as {symbol: {"qty": int, "entry": float}}.
    """
    symbols = [s for s, b in all_bars.items() if b is not None]
    if not symbols:
        raise ValueError("No valid bars to backtest.")
    ...
    for i, date in enumerate(common_idx):
        if i < 50:  # warm-up (need 50 bars for SMA50 etc.)
            equity_curve.append({"date": date, "equity": cash})
            continue

        sliced = {sym: all_bars[sym].loc[:date].copy() for sym in symbols}

        # ── Stop-loss / Take-profit exits ─────────────────────────────────────
        # NOTE: DipRider manages its own exits via signals (hard stop + trail).
        #       The engine-level stop/take-profit only applies to other strategies.
        to_close = []
        for sym, pos in positions.items():
            if sym not in sliced or sliced[sym].empty:
                continue
            curr_price = float(sliced[sym]["close"].iloc[-1])
            chg        = (curr_price - pos["entry"]) / pos["entry"]
            # Skip engine-level exits for DipRider (it handles its own)
            if pos.get("strategy") == "dip_rider":
                continue
            if chg <= -stop_loss_pct:
                to_close.append((sym, curr_price, "stop_loss", chg))
            elif chg >= take_profit_pct:
        ...
        for sig in signals:
            sym    = sig["symbol"]
            action = sig["signal"]
            if sym not in sliced or sliced[sym] is None or sliced[sym].empty:
                continue
            price    = float(sliced[sym]["close"].iloc[-1])
            strength = sig.get("strength", 1.0)
            ...
            if action == "buy" and sym not in positions:
                ...
                if cost <= cash:
                    ...
                    positions[sym] = {
                        "qty":      qty,
                        "entry":    price,
                        "strategy": strategy.__class__.__name__.lower().replace("strategy", ""),
                    }
                    trades.append({"date": date, "symbol": sym, "action": "buy",
                                   "price": price, "qty": qty, "pnl": 0})
    ...
    # Close open positions at final price
    for sym, pos in positions.items():
        if sym in sliced and not sliced[sym].empty:
            price = float(sliced[sym]["close"].iloc[-1])
            pnl   = (price - pos["entry"]) * pos["qty"]
...
def compute_metrics(results: dict) -> dict:
    ...
    tr_df    = results["trades"]
    n_trades = len(tr_df[tr_df["action"].isin(
        ["sell", "stop_loss", "take_profit", "close_eob"]
    )]) if not tr_df.empty else 0
    wins     = int((tr_df["pnl"] > 0).sum()) if not tr_df.empty else 0
    win_rate = wins / n_trades if n_trades > 0 else 0
    calmar   = ann_ret / abs(max_dd) if max_dd != 0 else 0.0
...
def print_report(strategy_name: str, symbols: list,
    ...
    print("-" * 60)
    if not tr_df.empty:
        print(f"  Total P&L       : ${tr_df['pnl'].sum():>+,.2f}")
        print(f"\n  Last 5 trades:")
        for _, row in tr_df.tail(5).iterrows():
            sign = "+" if row["pnl"] >= 0 else ""
            print(f"    {str(row['date'])[:10]}  {row['symbol']:6s}  "
...
def main():
    ...
    log.info(f"Backtesting [{args.strategy}] on {args.symbols} | {args.days}d | {tf}")

    fetch_syms = list(args.symbols)
    if args.strategy == "rs_breakout" and "SPY" not in fetch_syms:
        fetch_syms.append("SPY")

    all_bars = fetch_history(fetch_syms, args.days,
                             source=args.source, timeframe=tf, bar_limit=bar_limit)

    strategy = strategy_cls()

    # Strategy-specific stop/take-profit overrides
    if args.strategy == "dip_rider":
        stop_loss   = getattr(config, "DIPRIDER_HARD_STOP_PCT", 0.25)
        take_profit = 999.0   # DipRider manages its own exit — no engine take-profit
    else:
        stop_loss   = config.STOP_LOSS_PCT
        take_profit = config.TAKE_PROFIT_PCT
    ...
    if not results["trades"].empty:
        results["trades"].to_csv("logs/backtest_trades.csv", index=False)
    log.info("Saved to logs/")

    from utils.report_generator import generate_report
    generate_report(strategy_name=args.strategy, symbols=args.symbols,
                    results=results, metrics=metrics, days=args.days)
